...
To complement the statistical analysis, I implemented a supervised machine learning model to **predict customer gender** based on their purchase behavior. The goal was to investigate whether variables such as product category, purchase hour, unit price, and city could be predictive of gender.

---

###  Objective

The aim was to build a binary classification model that classifies customers as **Male** or **Female** using their purchase-related features. This can be useful for segmentation, personalized marketing, and targeted recommendations.
...
- It handles both **categorical** and **numerical** variables effectively.
- It is **robust to overfitting** and capable of modeling complex, non-linear patterns.
- It provides **feature importance scores**, helping explain which variables influence predictions.
- It was one of the main supervised learning techniques covered in class, making it an appropriate method for this course.

---

### Features Used

The input features used in the model were:

- `category`: Product category
- `city`: City where the order was placed
- `day_of_week`: Day of the purchase
- `is_weekend`: Whether the purchase was made on a weekend
- `unit_price`: Price of the item
- `hour`: Time of day when the order was placed

The target variable was `guessed_gender`, filtered to include only `"Female"` and `"Male"` entries (i.e., `"Unknown"` entries were removed).

---

###  Preprocessing

- Categorical features were transformed using **One-Hot Encoding** with `handle_unknown='ignore'` to manage unseen categories in the test set.
- Numerical features were passed through as-is.
- The dataset was split into training and test sets using `train_test_split`.
- All processing and model training were bundled into a `Pipeline` to ensure reproducibility and code clarity.

---

### Results

- The model achieved a **test accuracy of ~66%**.
- The **confusion matrix** showed that the model was slightly better at predicting **Male** purchases compared to **Female**.
- This suggests that gender-related patterns exist in the data, but may be influenced by label noise due to inferred gender values.

---

### Limitations

- The `guessed_gender` labels were inferred using name-based heuristics and may contain errors.
- Some categories (e.g., cities) in the test set did not appear in the training set, which may affect prediction performance.
- The dataset covers only one month of sales, limiting behavioral diversity.

---

### Future Work

- Experiment with other models (e.g., Logistic Regression, XGBoost) for comparison.
- Perform hyperparameter tuning using `GridSearchCV`.
- Incorporate more behavioral signals (e.g., browsing time, basket size) if available.
- Expand the dataset to include a full year to better generalize across seasonal behaviors.
